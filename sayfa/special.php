...
<form action="" method="get">
          <section class="row text-center placeholders">
            <div class="col-6 col-sm-3 placeholder">
				<img src="../veri/spesiyal/1.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
				<h4>Gourmet Collection Spesiyal Çikolata Kahverengi Kutu 252g</h4>             
				
				<span class="badge badge-success">39.95₺</span></p>
								<h5><input  class="btn btn-primary"type="submit" name="kaydet1" value="Sepete Ekle"></h5>
            </div>	
            <div class="col-6 col-sm-3 placeholder">
              <img src="../veri/spesiyal/2.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
              <h4>Mantar Çikolata 256g</h4>
              
				<span class="badge badge-success">27.90₺</span></p>
							<h5><input  class="btn btn-primary"type="submit" name="kaydet2" value="Sepete Ekle"></h5>
            </div> 
			<div class="col-6 col-sm-3 placeholder">
              <img src="../veri/spesiyal/3.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
              <h4>Taste of Passion Tutkunun Tadı Spesiyal Çikolata 187g</h4>

				<span class="badge badge-success">24.95₺</span></p>
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="Gourmet Spesiyal Cikolata Kahverengi Kutu 252g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="Mantar Cikolata 256g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="Taste of Passion Tutkunun Tadi Cikolata 187g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
