...
<form action="" method="GET">
          <section class="row text-center placeholders">
            <div class="col-6 col-sm-3 placeholder">
				<img src="../veri/madlen/1.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
				<h4>Bohem Madlen Çikolata Metal Kutu 250g</h4>             
				
				<span class="badge badge-success">41.95₺</span></p>
				<h5><input  class="btn btn-primary"type="submit" name="kaydet1" value="Sepete Ekle"></h5>
            </div>	
            <div class="col-6 col-sm-3 placeholder">
              <img src="../veri/madlen/2.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
              <h4>Medallion Collection Prestige Madlen Çikolata 500g</h4>
              
				<span class="badge badge-success">50₺</span></p>
				<h5><input  class="btn btn-primary"type="submit" name="kaydet2" value="Sepete Ekle"></h5>
            </div>  
			<div class="col-6 col-sm-3 placeholder">
              <img src="../veri/madlen/3.jpg" width="200" height="200" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail">
              <h4>VIP Madlen Çikolata Gümüş Kutu 500g</h4>
              
				<span class="badge badge-success">45.95₺</span></p>
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="Bohem Madlen Cikolata Metal Kutu 250g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="Medallion Collection Prestige Madlen Cikolata 500g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
...
$sonuc = $baglanti->query($sorgu);
			...
			if($sonuc->num_rows>0)
			{
				while($kayit = $sonuc ->fetch_assoc()) // fetch bir kayit geliyor sırayla diğerleride
				{
					...
					if($kayit["adi"]=="VIP Madlen Cikolata Gumus Kutu 500g")
					{
					$id=$kayit["id"];
					$adi=$kayit["adi"];
					$fiyat= $kayit["fiyat"];
